...
__zic_fzf_prog() {
    [ -n "$TMUX_PANE" ] && [ "${FZF_TMUX:-0}" != 0 ] && [ ${LINES:-40} -gt 15 ] \
        && echo "fzf-tmux -d${FZF_TMUX_HEIGHT:-40%}" || echo "percol --match-method pinyin"
}
...
__zic_matched_subdir_list_all() {
    ...
    if [[ "$1" == */ ]]; then
        ...
        find -L "$dir" -mindepth 1 -maxdepth 1 2>/dev/null \
            | cut -b $(( ${length} + 2 ))- | command sed '/^$/d' | while read -r line; do
            if [[ "${line[1]}" == "." ]]; then
                continue
            fi
...
_zic_list_generator_all() {
    __zic_matched_subdir_list_all "${(Q)@[-1]}" | sort
}
